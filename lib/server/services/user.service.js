...
export async function listUsers({ page, limit, search, role, status, sortBy, order }) {
...
/**
 * Calculate impact metrics based on mission category and XP earned
 */
function calculateImpactFromMission(category, xpEarned) {
  const baseValue = Math.ceil(xpEarned * 0.1); // 10% of XP as base impact value
  ...
  // Map mission category to impact metrics
  switch (category) {
    case "CLIMATE":
      impact.carbonSaved = baseValue;
      break;
    case "WATER":
      impact.waterSaved = baseValue;
      break;
    case "WASTE":
      impact.wasteClassified = baseValue;
      break;
    case "BIODIVERSITY":
      impact.speciesLearned = baseValue;
      break;
    case "COASTAL":
      impact.mangroveRestored = baseValue;
      break;
    case "OCEAN":
      impact.waterSaved = baseValue;
      break;
    case "TRANSPORT":
      impact.carbonSaved = baseValue * 0.8;
      impact.waterSaved = baseValue * 0.2;
      break;
    default:
      // For any other category, split evenly
      const split = Math.ceil(baseValue / 5);
      impact.carbonSaved = split;
      impact.waterSaved = split;
      impact.wasteClassified = split;
      impact.speciesLearned = split;
      impact.mangroveRestored = split;
  }
...
/**
 * Aggregate impact data from all mission completions
 */
function aggregateImpact(missionCompletions) {
  const totalImpact = {
    carbonSaved: 0,
    waterSaved: 0,
    wasteClassified: 0,
  ...
  missionCompletions.forEach((completion) => {
    ...
    totalImpact.carbonSaved += missionImpact.carbonSaved;
    totalImpact.waterSaved += missionImpact.waterSaved;
    totalImpact.wasteClassified += missionImpact.wasteClassified;
    totalImpact.speciesLearned += missionImpact.speciesLearned;
    totalImpact.mangroveRestored += missionImpact.mangroveRestored;
  });
...
/**
 * Convert impact metrics to tree equivalents
 * Based on environmental impact calculations
 */
function calculateTreeEquivalent(impact) {
  // Rough conversions:
  // 1 tree saves ~21kg CO2 per year
  // 1 tree saves ~1000L water per year
  // etc.
  const treeCarbonEquivalent = Math.ceil(impact.carbonSaved / 21);
  const treeWaterEquivalent = Math.ceil(impact.waterSaved / 1000);
  const treeBiodiversityEquivalent = Math.ceil(impact.speciesLearned / 2);
  
  return Math.max(
    treeCarbonEquivalent,
    treeWaterEquivalent,
    treeBiodiversityEquivalent,
    Math.ceil(impact.mangroveRestored * 0.5)
  );
}
