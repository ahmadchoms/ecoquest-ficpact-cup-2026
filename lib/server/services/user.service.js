 ...
 */
function formatActivityHistory(missionCompletions, limit = 4) {
...
export async function getUserDashboardData(email) {
  ...
  // Transform missionCompletions to array of keys for frontend store
  const completedMissionKeys = user.missionCompletions.map(
    (mc) => `${mc.mission.id}:${mc.mission.provinceId}`
  );

  // Hapus missionCompletions dari response setelah diproses
  ...
  return {
    ...
    completedMissions: completedMissionKeys, // Array of "${missionId}:${provinceId}"
 ...
 * Returns array of itemIds that user has purchased
 */
export async function getUserPurchasedShopItems(email) {
  const user = await prisma.user.findUnique({
    where: { email },
    select: {
      id: true,
...
/**
 * Get user's profile data (for user's own profile page)
 */
...
/**
 * Update user's profile (name, bio, profileImage)
 */
export async function updateUserProfile(email, data) {
  const { name, bio, profileImage, username } = data;
...
/**
 * Get navbar user data (name, xp, points, level, profileImage)
 */
export async function getNavbarUserData(email) {
  return prisma.user.findUnique({
    where: { email },
    select: USER_LIST_SELECT,
  });
 ...
 * Returns the current owner's email if taken, null if available
 */
export async function checkUsernameAvailability(username, currentUserEmail) {
  const existingUser = await prisma.user.findUnique({
    where: { username: username.toLowerCase() },
    select: { email: true },
  });

  // If username doesn't exist OR belongs to current user, it's available
  const isAvailable = !existingUser || existingUser.email === currentUserEmail;

  return {
    username,
    available: isAvailable,
    owner: existingUser?.email || null,
  };
}

/**
 * Get user's rank in the leaderboard
 * Returns rank and user data if user is not in top 10
 */
export async function getUserRank(email) {
  ...
  return {
    ...user,
    rank: rankPosition + 1, // +1 because count is 0-indexed
  };
}
