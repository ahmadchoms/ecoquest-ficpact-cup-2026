...
const COLORS = ['#0ea5e9', '#06b6d4', '#22d3ee', '#67e8f9', '#a5f3fc', '#0284c7'];

export default function WaterConservation({ province, mission, onComplete, onBack }) {
  ...
  const steps = [
    {
      title: "🚿 Mandi & Toilet",
      fields: [
        { key: 'bathMin', label: 'Durasi mandi (menit)', min: 1, max: 30, emoji: '🚿' },
        { key: 'flushCount', label: 'Flush toilet (kali/hari)', min: 1, max: 10, emoji: '🚽' },
      ],
      toggles: [
    ...
    },
    {
      title: "🍳 Dapur & Cuci",
      fields: [
        { key: 'cookingPortions', label: 'Porsi masak per hari', min: 0, max: 10, emoji: '🍳' },
        { key: 'washMachine', label: 'Cuci mesin (kali/minggu)', min: 0, max: 7, emoji: '👕' },
        { key: 'plantWater', label: 'Siram tanaman (liter/hari)', min: 0, max: 30, emoji: '🌱' },
      ],
      toggles: [
        { key: 'useTapDish', labelOn: 'Kran Air', labelOff: 'Wastafel Penuh', emoji: '🍽️' },
      ]
    },
  ];
  ...
  const handleSubmit = () => {
    const tips = [];
    if (inputs.useShower && inputs.bathMin > 10) tips.push("Kurangi durasi shower — setiap menit menghemat 12 liter air");
    if (inputs.useTapDish) tips.push("Cuci piring pakai wastafel penuh bisa hemat hingga 50 liter per sesi");
    if (inputs.washMachine > 4) tips.push("Kumpulkan cucian dan cuci seminggu 2-3 kali saja");
    if (inputs.plantWater > 10) tips.push("Gunakan air bekas cucian untuk menyiram tanaman");
    if (tips.length === 0) tips.push("Penggunaan airmu sudah sangat hemat — pertahankan!");
  ...
  if (result) {
    const chartData = Object.entries(result.breakdown)
      .filter(([, v]) => v > 0)
      .map(([key, value]) => ({
        name: key === 'bathing' ? 'Mandi' :
              key === 'toilet' ? 'Toilet' :
              key === 'kitchen' ? 'Dapur' :
              key === 'cooking' ? 'Masak' :
              key === 'laundry' ? 'Cuci' : 'Tanaman',
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
        ...
        <div className="eco-card p-5 text-center">
          <p className="text-sm text-gray-500 mb-1">Total penggunaan per hari</p>
          <p className={`font-heading text-4xl font-bold ${
            result.rating === 'excellent' ? 'text-green-500' :
            result.rating === 'good' ? 'text-blue-500' :
            result.rating === 'average' ? 'text-yellow-500' :
            'text-red-500'
          }`}>
          ...
          <div className={`inline-block mt-2 px-3 py-1 rounded-full text-sm font-medium ${
            ...
            {result.total <= result.nationalAverage ? '✅ Di bawah rata-rata' : '⚠️ Di atas rata-rata'}
          </div>
        </div>

        <div className="eco-card p-5">
          <p className="text-sm text-gray-500 mb-3 text-center">Breakdown Penggunaan</p>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="eco-card p-4 text-center">
            ...
            <p className="font-bold text-lg text-gray-800">{result.bottleEquivalent}</p>
            <p className="text-xs text-gray-500">botol air 600ml</p>
          </div>
          <div className="eco-card p-4 text-center">
            <p className="text-2xl mb-1">📅</p>
            <p className="font-bold text-lg text-gray-800">{result.yearlyUsage.toLocaleString()}</p>
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <button onClick={step > 0 ? () => setStep(step - 1) : onBack} className="flex items-center gap-1 text-gray-500 hover:text-gray-700 text-sm">
      ...
      <motion.div key={step} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
        <h3 className="font-heading text-xl font-bold text-gray-800 mb-4">{currentStep.title}</h3>

        {currentStep.toggles?.map(toggle => (
          <div key={toggle.key} className="eco-card p-4 mb-3">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-gray-700">{toggle.emoji} Tipe</span>
