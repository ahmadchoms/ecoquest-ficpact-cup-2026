...
export default function MangroveSimulator({ province, mission, onComplete, onBack }) {
  ...
  const restoredPercent = totalDamaged > 0 ? Math.round((plantedCount / totalDamaged) * 100) : 0;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="eco-card p-4">
        <h3 className="font-heading font-bold text-gray-800 mb-1">🌊 Pantai {province.name}</h3>
                ...
                return (
                  <motion.button
                    ...
                    className={`w-9 h-9 md:w-10 md:h-10 rounded-lg flex items-center justify-center text-sm transition-all
                      ${isPlanted ? 'bg-green-500 shadow-sm shadow-green-300' :
                        cell.type === 'water' ? 'bg-blue-400' :
                        cell.type === 'sand' ? 'bg-amber-200' :
        ...
        <div className="mt-3 flex items-center gap-2">
          ...
          <span className="text-xs text-gray-500">{restoredPercent}% pulih</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        ...
        <div className="eco-card p-3">
          <p className="text-lg font-bold text-amber-600">{(plantedCount * 0.5).toFixed(1)}</p>
